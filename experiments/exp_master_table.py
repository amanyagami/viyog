...
Sources (all pre-computed, CPU only, no re-extract):
  - full_eval_<ds>_summary.csv     logit/Viyog T1/T2/T3 directionless + fpr95 + aupr + deployable
...
COST = {
    ...
    "KLMatching":     (0.05, 100., 100., 100.),
    "MSP":            (0.004, 100., 100., 100.),
    "MaxLogit":       (0.004, 100., 100., 100.),
    "Mahalanobis":    (7588., 100., 100., 100.),
    "KNN":            (25600., 100., 100., 100.),
    "ViM":            (7300., 100., 100., 100.),
}
FAMILY = {
    "ViyogD_tv_dorm": "Viyog (first-conv)", "ViyogD_hf_dorm": "Viyog (first-conv)",
    "Viyog_Linf": "Viyog (first-conv)",
    "Energy": "logit", "Entropy": "logit", "GEN": "logit", "KLMatching": "logit",
    "MSP": "logit", "MaxLogit": "logit",
    "Mahalanobis": "distance (feature)", "KNN": "distance (feature)", "ViM": "distance (feature)",
}
...
def main() -> None:
    ...
    # full_eval is long (one row per method x task); pivot the directionless AUROC + fpr95
    piv = fe.pivot_table(index="method", columns="task", values="mean_dl")
    fpr = fe.pivot_table(index="method", columns="task", values="mean_fpr95")
    dep = fe.groupby("method")["deployable"].all()
    ...
    for m in piv.index:
        rows.append({
            ...
            "recall@5%FPR_T3": round(1 - fpr.loc[m].get("T3", np.nan), 3),  # TPR at 5% FPR ~ 1-fpr95 proxy
            "deployable": bool(dep.get(m, False)),
            "dir_consistent": bool(dc.get(m, False)),
        })

    # distance baselines: average per-model T1/T2/T3 from baselines_feature
    bf_path = A / f"baselines_feature_{args.dataset}.csv"
    if bf_path.exists():
        bf = pd.read_csv(bf_path)
        g = bf.groupby("method").agg(T1=("T1_ID_OOD", "mean"), T2=("T2_ID_ADV", "mean"),
                                     T3=("T3_OOD_ADV", "mean")).round(3)
        for m in g.index:
            rows.append({
                "method": m,
                "T1_ID_OOD": g.loc[m, "T1"], "T2_ID_ADV": g.loc[m, "T2"],
                "T3_OOD_ADV": g.loc[m, "T3"], "recall@5%FPR_T3": np.nan,
                "deployable": True, "dir_consistent": True,
            })

    df = pd.DataFrame(rows)
    # attach cost
    cost = pd.DataFrame.from_dict(COST, orient="index",
    ...
    order = ["method", "family", "T1_ID_OOD", "T2_ID_ADV", "T3_OOD_ADV", "recall@5%FPR_T3",
             "deployable", "dir_consistent", "state_mem_KB", "compute_%fwd", "cpu_lat_%fwd", "accel_energy_%fwd"]
    df = df[order]
    ...
    # markdown
    md = [f"# Master comparison — Viyog vs all pytorch-ood baselines ({args.dataset})",
          "",
          "All AUROC directionless `max(a,1-a)`, averaged over the model zoo. Cost columns: "
          "detector-state memory (KB), and compute / CPU-latency / accelerator-energy as **% of a full "
          "forward** (Viyog = first-conv only).",
          "",
    ...
    for _, r in df.iterrows():
        ...
        md.append(f"| {r.method} | {r.family} | {f(r.T1_ID_OOD)} | {f(r.T2_ID_ADV)} | "
    ...
    md += ["", "## Headline",
           ...
           f"(logit non-deployable/dir-inconsistent on T2).",
           f"- **T3 (OOD-vs-ADV)** — Viyog-tv {df.set_index('method').loc['ViyogD_tv_dorm','T3_OOD_ADV']:.3f}; "
           f"logit ties on AUROC but at full-forward cost; distance best = ViM "
